...
router.post('/', async(req,res,next) =>
{
  ...
  const usermode="admin"
  console.log(username)
  console.log(password)
  try {
        //finding the matching user
          const user = await User.findOne({ "email": username,"usertype": usermode});
          // $or: [{
          //   "email": username
          // }, {
          //   "IFID": username
          ...
          const admintoken=jwt.sign({_id:user._id,IFID:user.IFID,useType:user.usertype},process.env.TOKENSECRET)
          cookieOptions={
            secure:false,
            httpOnly:true,
            expires: new Date(
